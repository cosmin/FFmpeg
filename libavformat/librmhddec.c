...
static void rmhd_reset_head(AVFormatContext *s)
{
...
static void rm_read_metadata(AVFormatContext *s, AVIOContext *pb, int wide)
{
    ...
    for (i=0; i<FF_ARRAY_ELEMS(ff_librmhd_metadata); i++) {
        ...
        av_dict_set(&s->metadata, ff_librmhd_metadata[i], buf, 0);
    }
}

RMStream *ff_librmhd_alloc_rmstream (void)
{
...
void ff_librmhd_free_rmstream (RMStream *rms)
{
                                     ...
                                     AVStream *st, RMStream *ast, int read_all)
{
    ...
    if (version == 3) {
        ...
        avio_rb16(pb); /* ??? */
        if (version == 5) {
            avio_rb16(pb);  //codec frame_size

            //For sample_rate 32-bit value, the upper 16 bits are the integer part and the
            //lower 16 bits are the fractional part. Now we discard the fractional part. So
            //we use avio_rb16 rather than avio_rb32
            //avio_rb32(pb); //sample_rate
            avio_rb16(pb); //sample_rate
            avio_rb16(pb);
        }
        //st->codecpar->sample_rate = avio_rb32(pb); //actual sample rate
        ...
        st->codecpar->codec_id   = ff_codec_get_id(ff_librmhd_codec_tags,
...
int ff_librmhd_read_mdpr_codecdata (AVFormatContext *s, AVIOContext *pb,
                                   ...
                                   int codec_data_size, const uint8_t *mime)
{
    ...
    if (v == MKTAG(0xfd, 'a', 'r', '.')) {
        ...
        fps =  ((float) ((((float)fps_rd)/ (float)(1L <<16))));
...
{
    AVIOContext *pb = s->pb;
    unsigned int size, n_pkts, str_id, n, pts;
    uint64_t next_off=0, pos = 0;
    AVStream *st;
    int version = 0;
...
static int rm_read_header_old(AVFormatContext *s)
{
    ...
    if (!st)
        return -1;
    st->priv_data = ff_librmhd_alloc_rmstream();
    if (!st->priv_data)
        return AVERROR(ENOMEM);
    return rm_read_audio_stream_info(s, s->pb, st, st->priv_data, 1);
}

static int librmhd_read_close(AVFormatContext *s);
static int librmhd_read_header(AVFormatContext *s)
{
    RMDemuxContext *rm = s->priv_data;
    AVStream *st;
    AVIOContext *pb = s->pb;
    ...
    for(;;) {
        ...
        case MKTAG('P', 'R', 'O', 'P'):
            ...
            flags = avio_rb16(pb); /* flags , 9 is the value in test file*/
            break;
        case MKTAG('C', 'O', 'N', 'T'):
            ...
            if (v == MKBETAG('M', 'L', 'T', 'I')) {
                ...
                for (i = 0; i < number_of_mdpr; i++) {
                    ...
                    if (i > 0) {
                        ...
                        st2->priv_data = ff_librmhd_alloc_rmstream();
                        if (!st2->priv_data)
                            return AVERROR(ENOMEM);
                    } else
                        st2 = st;
                    ...
                    size2 = avio_rb32(pb);
                    if (ff_librmhd_read_mdpr_codecdata(s, s->pb, st2, st2->priv_data,
                                                  size2, mime) < 0)
                        goto fail;
                }
                ...
                avio_skip(pb, -4);
                if (ff_librmhd_read_mdpr_codecdata(s, s->pb, st, st->priv_data,
                                              size, mime) < 0)
                    goto fail;
            }
        ...
        *pos= avio_tell(pb) - 3;
        if(rm->remaining_len > 0){
            ...
            *flags= 0;
            ...
            num = avio_rb16(pb);
            timestamp_rd = avio_rb32(pb);
            *timestamp = timestamp_rd;
...
int
ff_librmhd_parse_packet (AVFormatContext *s, AVIOContext *pb,
...
int
ff_librmhd_retrieve_cache (AVFormatContext *s, AVIOContext *pb,
...
static int librmhd_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    RMDemuxContext *rm = s->priv_data;
    ...
    for (;;) {
        if (rm->audio_pkt_cnt) {
            ...
            res = ff_librmhd_retrieve_cache(s, s->pb, st, st->priv_data, pkt);
            if(res < 0)
                return res;
            flags = 0;
        } else {
            ...
            res = ff_librmhd_parse_packet (s, s->pb, st, st->priv_data, len, pkt,
                                      &seq, flags, timestamp);
            if (res < -1)
            {
                if(s->pb->eof_reached)
...
static int librmhd_read_close(AVFormatContext *s)
{
    ...
    for (i=0;i<s->nb_streams;i++)
        ff_librmhd_free_rmstream(s->streams[i]->priv_data);

    return 0;
}

static int librmhd_probe(AVProbeData *p)
{
    /* check file header ? need FIX */
...
static int64_t librmhd_read_dts(AVFormatContext *s, int stream_index,
...
static int librmhd_read_seek(AVFormatContext *s, int stream_index,
                        int64_t pts, int flags)
{
    ...
    if (!pts && s->pb) {
        if(avio_seek(s->pb, s->internal->data_offset, SEEK_SET) < 0)
            return -1;
        rmhd_reset_head(s);
        return 0;
    }

    if (st && st->index_entries) {
...
AVInputFormat ff_librmhd_demuxer = {
    .name           = "rmhd",
    .long_name      = NULL_IF_CONFIG_SMALL("RealMedia HD Container"),
    .priv_data_size = sizeof(RMDemuxContext),
    .read_probe     = librmhd_probe,
    .read_header    = librmhd_read_header,
    .read_packet    = librmhd_read_packet,
    .read_close     = librmhd_read_close,
    .read_timestamp = librmhd_read_dts,
    .read_seek      = librmhd_read_seek,
};
