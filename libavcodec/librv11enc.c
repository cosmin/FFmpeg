...
static av_cold int librmhdenc_init(AVCodecContext *avctx)
{
    ...
    if(SUCCEEDED(res))
    {
        ...
        if(enctx->bVbrBitrate&&!(avctx->flags&CODEC_FLAG_PASS1))
        {
        ...
        if(SUCCEEDED(res) && (enctx->rvenc_param.eEncodeType == ENCODE_VBR_BITRATE))
        {
            ...
            if(avctx->flags&CODEC_FLAG_PASS2 && parm==2)
            {
            ...
            else if(parm==1 && !(avctx->flags&CODEC_FLAG_PASS2) && !(avctx->flags&CODEC_FLAG_PASS1)) 
            {
                parm = 0;
            }
        ...
        if(avctx->flags&CODEC_FLAG_PASS1||avctx->flags&CODEC_FLAG_PASS2)
        {
...
{
    .name                  = "librv11enc",
    .long_name             = NULL_IF_CONFIG_SMALL("librv11enc RV60"),
